...
const TextToSpeech = () => {
  ...
  const wordToRead = () => {
    ...
    let textToRead = `${result.word}.`; // Start by reading the word.
  
    // Include the phonetic representation if it exists
    if (result.phonetic) {
      textToRead += ` Pronounced as: ${result.phonetic}.`;
    }
  
    // Iterate over the meanings and construct the sentences for part of speech and definition
    result.meanings.forEach((meaning) => {
      ...
      meaning.definitions.forEach((def) => {
        textToRead += ` Definition: ${def.definition}.`;
